...
func printMeasurementGroup(measurementGroup: MeasurementGroup) {
    ...
    var sortedMeasurements = map(measurementGroup.resultSets) {
        return $0
    }
    ...
    let maxTagLength = count(formatDuration(Duration(nanoseconds: 0)))
    let paddedTags = tags.map { leftPad($0, toLength: maxTagLength) }

    // Print the title line
    let titleLine = lineForElements([paddedGroupName] + paddedTags)
    let separatorLine = separatorLineWithLength(count(titleLine))
    println(titleLine)
...
private func leftPad(string: String, toLength length: Int) -> String {
    ...
    let padding = String(count: length - count(shortenedString), repeatedValue: paddingCharacter)

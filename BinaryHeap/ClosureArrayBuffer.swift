...
// Pretty much an exact copy of the regular ArrayBuffer but with added storage for the comparison
// closure.
...
private final class ArrayBufferStorage<Element> : NonObjectiveCBase {
    ...
    private init(isOrderedBefore: (Element, Element) -> Bool) {
        elements = nil
        count = 0
        capacity = 0
    ...
    private init(capacity: Int, sourceBuffer buffer: ArrayBufferStorage<Element>, destroySourceContents: Bool = false) {
        ...
        self.isOrderedBefore = buffer.isOrderedBefore
...
internal struct ClosureArrayBuffer<Element> {
    ...
    init(minimumCapacity: Int = 0, isOrderedBefore: (Element, Element) -> Bool) {
        storage = ArrayBufferStorage(isOrderedBefore: isOrderedBefore)
